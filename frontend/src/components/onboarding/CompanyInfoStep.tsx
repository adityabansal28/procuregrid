...
export function CompanyInfoStep({ data, onChange, onNext, onSignOut, signingOut }: Props) {
  ...
  function handleNext(e: React.FormEvent) {
    ...
    const gst = data.gstNumber.trim() ? validateGST(data.gstNumber) : "GSTIN is required";
    const pan = data.panNumber.trim() ? validatePAN(data.panNumber) : "PAN is required";
    setGstError(gst);
    setPanError(pan);
    if (gst || pan) return;
    onNext();
  ...
  return (
    <form className="grid gap-4 md:grid-cols-2" onSubmit={handleNext}>
      ...
      <div className="space-y-2">
        ...
        <Select value={data.businessType} onValueChange={(v) => set("businessType", v)}>
          <SelectTrigger>
